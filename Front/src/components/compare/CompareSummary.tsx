  ...
  return (
    <section dir={i18n.dir()} className="mx-auto w-full  max-w-[1200px] px-4 pb-8">
      <div className="rounded-[20px] bg-[#021F38] px-8 py-8 text-center text-white">
        ...
        {/* Winner label */}
        <p className="text-[14px] pb-1 text-white/60">
          {isTie
            ? t("comparePage.tieLabel")
            : t("comparePage.winnerLabel")}
        </p>

        {/* Winner name */}
        <h2 className="mt-1 pb-3  text-[28px] font-extrabold text-white md:text-[34px]">
          {isTie ? `${car1Name} - ${car2Name}` : winnerName}
        </h2>

        {/* Score */}
        <p className="mt-2 text-[15px] font-bold text-[var(--brand-secondary-color)]">
          {isTie
            ? t("comparePage.tieScore", { score: winnerScore })
            : t("comparePage.winnerScore", { score: winnerScore })}
        </p>
        <p className="mt-0.5 text-[13px] text-white/50">
          {isTie
            ? t("comparePage.tieScoreDetail")
        ...
        <div className="mt-6 flex items-center justify-center gap-3">
          <button
            type="button"
            onClick={() =>
              (isTie ? car1Slug : winnerSlug) &&
              navigate(`/cars/${isTie ? car1Slug : winnerSlug}`)
            }
          ...
          <button
            type="button"
            onClick={() =>
              (isTie ? car2Slug : winnerSlug) &&
              navigate(`/cars/${isTie ? car2Slug : winnerSlug}`)
            }
