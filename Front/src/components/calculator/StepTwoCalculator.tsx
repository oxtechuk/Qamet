  ...
  return (
    <div dir={direction} className="space-y-4">
      ...
      <div className="rounded-[20px] border border-[#E5E7EB] bg-white px-6 py-6 shadow-sm">
        <h2 className="mb-5 text-start text-[18px] font-extrabold text-[#111827]">{t("financeCalculator.step2.financeDetails")}</h2>

        <div className="mb-6">
          <div className="mb-2 flex items-center justify-between text-[13px]">
            <span className="text-[#9CA3AF]">{t("financeCalculator.step2.carPrice")}</span>
            <span className="font-bold text-[#111827]">{carPrice.toLocaleString()} {riyal}</span>
          </div>
          <div dir="ltr">
            <input
              type="range"
              ...
              style={{ background: getSliderBg(carPrice, CAR_MIN, CAR_MAX) }}
            />
          </div>
        </div>
        ...
        <div className="mb-6">
          <div className="mb-2 flex items-center justify-between text-[13px]">
            <span className="text-[#9CA3AF]">{t("financeCalculator.step2.downPayment")} ({downPaymentPercent}%)</span>
            <span className="font-bold text-[#111827]">{downPayment.toLocaleString()} {riyal}</span>
          </div>
          <div dir="ltr">
            <input
              type="range"
              ...
              className="calc-slider"
              style={{ background: getSliderBg(downPaymentPercent, DP_MIN, DP_MAX) }}
            />
          </div>
        </div>

        <div>
          <p className="mb-3 text-start text-[13px] text-[#9CA3AF]">{t("financeCalculator.step2.financeTerm")}</p>
      ...
      <div className="rounded-[20px] bg-[#021F38] px-6 py-6 text-white">
        <p className="mb-4 text-start text-[12px] text-white/50">{t("financeCalculator.step2.summaryTitle")}</p>

        <div className="mb-5 grid grid-cols-2 divide-x divide-x-reverse divide-white/10">
          <div className="pe-4 text-center">
            <p className="text-[12px] text-white/50">{t("financeCalculator.step2.monthlyPayment")}</p>
            <p className="mt-1 text-[26px] font-extrabold text-[var(--brand-secondary-color)]">
              {monthlyPayment > 0 ? `${monthlyPayment.toLocaleString()} ${riyal}` : "—"}
            </p>
          </div>
          <div className="ps-4 text-center">
            <p className="text-[12px] text-white/50">{t("financeCalculator.step2.totalFinance")}</p>
        ...
        <div className="mb-5 space-y-2 border-t border-white/10 pt-4">
          <div className="flex justify-between text-[13px]">
            ...
            <span>{financeAmount > 0 ? `${financeAmount.toLocaleString()} ${riyal}` : "—"}</span>
          </div>
          <div className="flex justify-between text-[13px]">
            <span className="text-white/60">{t("financeCalculator.step2.totalInterest")}</span>
            <span>{calcResult?.total_interest ? `${calcResult.total_interest.toLocaleString()} ${riyal}` : "—"}</span>
          </div>
          <div className="flex justify-between text-[13px]">
